...
TF1 *GetRapDistribution(Int_t multMin = 1, Int_t multMax =8, Int_t iteration = 1)
{
  TString inputData = gSystem->GetFromPipe(Form("cat Mult-%dto%d/RapShapeIterations/iter-%d/values.txt", multMin, multMax, iteration));
...
TH1 *GetRapWeight(Int_t multMin = 1, Int_t multMax =8, Int_t iteration = 1, Bool_t drawIt = kFALSE)
{
  ...
  rapDistributionOld = GetRapDistribution(multMin, multMax, iteration - 1);
  rapIntegralOld = rapDistributionOld->Integral(-4, -2.5);

  rapDistributionNew = GetRapDistribution(multMin, multMax, iteration);
  ...
  if (drawIt)
  {
    ...
    lineAtOne->Draw();
    canWeightRap->SaveAs(Form("Mult-%dto%d/RapShapeIterations/iter-%d/RapWeight.pdf", multMin, multMax, iteration));
  }
  return histo;
}
//--------------------------------------------------------------------------------------------//
...
//--------------------------------------------------------------------------------------------//
//Simle funtion to get a TF1 from a given step
TF1 *GetPtDistribution(Int_t multMin = 1, Int_t multMax =8, Int_t iteration = 1)
{
  TString inputData = gSystem->GetFromPipe(Form("cat Mult-%dto%d/PtShapeIterations/iter-%d/values.txt", multMin, multMax, iteration));
  TObjArray *objInputData = inputData.Tokenize("\n");

  TF1 *ptDistribution = new TF1(Form("PtDistribution_%d", iteration), pTdist, 0, 20, 4);
...
//--------------------------------------------------------------------------------------------//
//The main function. It return a histogram with the weight values (TODO: return the weight as TF1 without converting it to TH1. This has shown some problems when used for weighting)
TH1 *GetPtWeight(Int_t multMin = 1, Int_t multMax =8, Int_t iteration = 1, Bool_t drawIt = kFALSE)
{

  ptDistributionOld = GetPtDistribution(multMin, multMax, iteration - 1);

  ptIntegralOld = ptDistributionOld->Integral(0, 20);

  ptDistributionNew = GetPtDistribution(multMin, multMax, iteration);
  ptIntegralNew = ptDistributionNew->Integral(0, 20);

  ptDistributionNewScaled = new TF1("pTgenrescaled", RescalePt, 0, 20, 0);
  TF1 *ptWeight = new TF1(Form("ptWeight_%d", iteration), DividePt, 0, 20, 0);
  ...
  if (drawIt)
  {
    ...
    padRatio->cd();
    TH1F *histoEmpty = new TH1F("empty", "", 10, 0, 20);
    ...
    lineAtOne->SetLineStyle(kDashed);
    lineAtOne->Draw();
    canWeightPt->SaveAs(Form("Mult-%dto%d/PtShapeIterations/iter-%d/PtWeight.pdf", multMin, multMax, iteration));
  }
