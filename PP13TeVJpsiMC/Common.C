...
void SetHistoStyle(TH1 *histoSomething, Int_t color, Int_t markerStyle, Float_t markerSize, TString xAxisTitle, TString yAxisTitle, Int_t rebinFactor, Bool_t isRatio)
{
  ...
  if (!isRatio)
  {
    ...
    histoSomething->GetYaxis()->SetLabelSize(0.045);
    histoSomething->GetYaxis()->SetTitleSize(0.045);
...
//-------------------------------------------------------------------------------------------------------------------------------------//
//For a given multiplicity bin, this will retrieve the latest iteration.
TString GetLatestIteration(int multMin = 1, int multMax = 8)
{
  TSystemDirectory filesDir(Form("Mult-%dto%d/AccEffi", multMin, multMax), Form("Mult-%dto%d/AccEffi", multMin, multMax));
...
const TString arrayColors[] = {"#d50000", "#4285F4", "#18FFFF", "#FF5722", "#F4B400", "#0F9D58", "#673AB7", "#FFABF5", "#FF690D", "#596E80"};
//-----------------------------------------------------------------------------------------------------------------------------///
