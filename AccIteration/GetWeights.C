...
{
  TString inputData = gSystem->GetFromPipe(Form("cat RapShapeIterations/iter-%d/values.txt", iteration));
...
TH1 *GetRapWeight(Int_t iteration = 1, Bool_t drawIt = kFALSE)
{
  ...
  if (drawIt)
  {
    ...
    lineAtOne->Draw();
    canWeightRap->SaveAs(Form("RapShapeIterations/iter-%d/RapWeight.pdf", iteration));
  }
  return histo;
}
//--------------------------------------------------------------------------------------------//
...
{
  TString inputData = gSystem->GetFromPipe(Form("cat PtShapeIterations/iter-%d/values.txt", iteration));
  TObjArray *objInputData = inputData.Tokenize("\n");
...
TH1 *GetPtWeight(Int_t iteration = 1, Bool_t drawIt = kFALSE)
{
  ...
  if (drawIt)
  {
    ...
    lineAtOne->SetLineStyle(kDashed);
    lineAtOne->Draw();
    canWeightPt->SaveAs(Form("PtShapeIterations/iter-%d/PtWeight.pdf", iteration));
  }
